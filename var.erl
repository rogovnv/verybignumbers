...
-export([start/0, stop/0, setvar/2, getvar/1, get_task_str/1, set_task_str/1]).
...
get_task_str(N) ->
  gen_server:call(?MODULE, {gets, N}).

set_task_str(List) ->
  gen_server:call(?MODULE, {sets, List}).

setvar(V, N) ->
  gen_server:cast(?MODULE, {set, V, N}).

getvar(V) ->
  gen_server:call(?MODULE, {get, V}).
